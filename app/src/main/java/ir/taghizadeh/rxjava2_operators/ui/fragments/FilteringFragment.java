...
public class FilteringFragment extends Fragment {
    ...
    @BindView(R.id.rv_filtering)
    RecyclerView rv_filtering;
    ...
    private void initRecyclerView() {
        rv_filtering.setLayoutManager(new LinearLayoutManager(getActivity()));
        gson = new GsonBuilder().create();
        jsonHelper = new JsonHelper();
        Model_Data model_data = gson.fromJson(jsonHelper.LoadJSONFromAsset("FilteringObservable.json", getActivity()), Model_Data.class);
        Adapter adapter = new Adapter(model_data);
        rv_filtering.setAdapter(adapter);
    }
